...
class Raspored {
  ...
  dajTrenutnuAktivnost(dateTime, grupa) {
    ...
    if (grupa !== "" || grupa !== " ") {
      trenutniDogadjaji.map((dogadjaj) => {
        if (dogadjaj.grupa === grupa) return dogadjaj.naziv + " " + datum.dajRazlikuUMinutamaOd(dogadjaj.end);
      });
    }
  ...
  dajSljedecuAktivnost(dateTime, grupa) {
    ...
    if (trenutniDogadjaji.length === 0) return "Nastava je gotova za danas";
    else if (trenutniDogadjaji.length === 1)
  ...
  dajPrethodnuAktivnost(dateTime, grupa) {
    ...
    trenutniDogadjaji.sort((a, b) => {
      return (
        a.danIndex - b.danIndex &&
        (a.startSati === b.startSati ? a.startMinute - b.startMinute : a.startSati - b.startSati)
      );
    });
